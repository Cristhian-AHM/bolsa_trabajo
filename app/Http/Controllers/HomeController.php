...
class HomeController extends Controller
{
    ...
    {
        $comprasmes=DB::select('SELECT month(c.created_at) as mes, count(c.id) as totalmes, c.name as name from offers c where c.status="ACTIVE" group by month(c.created_at) order by month(c.created_at) desc limit 12;');
        //dd($comprasmes);
